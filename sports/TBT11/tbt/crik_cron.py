...
def day_match_announce():
    try:
        ...
        for match in todays_match:
            ...
            if match.time <= datetime.time(16):
                match.announced = True
                announced_player = player_announce_crontab(match.id)
                return True
    except:pass
...
def night_match_announce():
    try:
        todays_match = Match.objects.filter(date = datetime.date.today(), live=False, end=False)
        for match in todays_match:
            if match.time <= datetime.time(20):
                match.announced = True
                player_announce_crontab(match.id)
...
def player_announce_crontab(id):
    try:
        ...
        teams_players = scorecard_soup.findAll(class_="cb-col cb-col-100 cb-minfo-tm-nm")

        for playing in teams_players:
            if 'Playing' in playing.text:
                ...
                for num_counter, team_players in enumerate(teams_players):
                    ...
                    for live_player in live_players_points:
                        ...
                        if len(team_players) == 11 and num_counter == 0:
                            ...
                            res = [ele for ele in live_player.player.full_name.split(' ') if(ele in team_players)]
